#!/usr/bin/env python3
"""Freeze fresh MS MARCO train queries before V13 candidate evaluation."""
...
def validate_source(
    ...
    if head != source_commit or status:
        raise ValueError("V13 fresh-query freeze requires a clean exact checkout")
...
def freeze(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.output_dir.exists() and any(args.output_dir.iterdir()):
        raise ValueError("Refusing to overwrite a non-empty V13 fresh-query freeze")
    ...
    expected_prior_count = int(fresh["expected_unique_excluded_qids"])
    ...
    atomic_json(
        manifest_path,
        {
            ...
            "status": "RARS_V13_FRESH_QUERIES_FROZEN_BEFORE_CANDIDATES",
    ...
    payload = {
        "schema_version": 1,
        "protocol_id": PROTOCOL_ID,
        "status": "RARS_V13_FRESH_QUERY_FREEZE_COMPLETE",
        "source_commit": args.source_commit,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--target-query-count", type=int, default=5000)
