#!/usr/bin/env python3
"""Build the frozen V12 Top-100 candidate/residual development bundle."""
...
from rars_v12_ca_rpq_core import PROTOCOL_ID, deterministic_fold_ids  # noqa: E402
...
def validate_source(
    ...
    if head != source_commit or status:
        raise ValueError("V12 bundle build requires a clean exact checkout")
    protocol = read_json(canonical)
    if protocol.get("protocol_id") != PROTOCOL_ID:
        raise ValueError("Unexpected V12 protocol")
    return protocol, {
...
def _load_query_freeze(
    ...
    if freeze.get("status") != "RARS_V12_FRESH_QUERY_FREEZE_COMPLETE":
        raise ValueError("Fresh query freeze is incomplete")
    if manifest.get("status") != "RARS_V12_FRESH_QUERIES_FROZEN_BEFORE_CANDIDATES":
    ...
    for relative in protocol["fresh_query_freeze"]["prior_qid_sources"]:
        value = read_json(repo_root / relative)
        prior.update(str(item) for item in value)
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.output_dir.exists() and any(args.output_dir.iterdir()):
        raise ValueError("Refusing to overwrite a non-empty V12 fresh bundle")
    ...
    manifest = {
        ...
        "status": "RARS_V12_FRESH_DEVELOPMENT_BUNDLE_FROZEN",
    ...
    complete = {
        "schema_version": 1,
        "protocol_id": PROTOCOL_ID,
        "status": "RARS_V12_FRESH_BUNDLE_COMPLETE",
        "source_commit": args.source_commit,
