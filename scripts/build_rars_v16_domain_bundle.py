#!/usr/bin/env python3
"""Build one outcome-free candidate/residual bundle for the RARS-v16 audit.
...
def _reject_forbidden_path(path: Path, label: str) -> None:
    ...
    if any(token in lowered for token in FORBIDDEN_PATH_TOKENS):
        raise ValueError(f"V16 refuses forbidden {label} path: {path}")
...
def _load_protocol(
    ...
    if head != source_commit or status:
        raise ValueError("V16 bundle build requires a clean exact checkout")
    ...
    if (
        ...
        raise ValueError("Unexpected V16 protocol identity or status")
    source_paths = (
        CANONICAL_PROTOCOL,
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    allowed_domains = protocol["data_policy"]["allowed_development_domains"]
    if args.domain_id not in allowed_domains:
        raise ValueError(f"Domain {args.domain_id!r} is not preregistered")
    if args.evidence_role not in ("fit", "evaluation"):
        raise ValueError("V16 domain role is not a permitted development role")
    if args.output_dir.exists() and any(args.output_dir.iterdir()):
        raise ValueError("Refusing to overwrite a non-empty V16 domain bundle")
    ...
    if queries.shape != (len(query_ids), args.dimension) or queries.dtype != np.float32:
        raise ValueError("Query vectors differ from the V16 shape/dtype contract")
    ...
    folds = np.asarray(
        [
            _deterministic_fold(query_id, args.domain_id, args.fold_count)
            for query_id in query_ids
        ],
        dtype=np.int64,
    )
    if len(np.unique(folds)) != args.fold_count:
        raise ValueError("Hash split left at least one V16 fold empty")
    ...
    complete = {
        "schema_version": 1,
        "protocol_id": PROTOCOL_ID,
        "status": "RARS_V16_DOMAIN_BUNDLE_COMPLETE",
        "source_commit": args.source_commit,
